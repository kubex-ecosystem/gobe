  ...
  return (
    <div className="relative mx-auto flex max-w-5xl flex-col-reverse gap-14 px-4 pb-24 pt-12 text-slate-700 transition-colors duration-300 sm:px-6 md:flex-row md:items-start md:pb-28 md:pt-16 dark:text-slate-300">
      <div className="md:w-1/2">
        <Badge tone="cyan">Portal de Autenticação</Badge>
        <h1 className="mt-5 text-3xl font-semibold text-slate-900 dark:text-white">Conceda acesso com governança Kubex</h1>
        <p className="mt-3 text-slate-600 dark:text-slate-300">
          Escolha o fluxo adequado para humanos, agentes ou integrações. Cada tentativa é auditável e respeita as diretivas do manifesto Kubex.
        </p>
        ...
        <div className="mt-8 space-y-4">
          <div className="flex items-start gap-3 rounded-2xl border border-cyan-600/15 bg-cyan-50/70 p-4 text-sm text-cyan-700 dark:border-cyan-500/30 dark:bg-slate-900/70 dark:text-cyan-200">
            ...
            <p>
              Tokens e credenciais são armazenados no núcleo GoBE. Defina escopos, TTLs e políticas de rotação por módulo.
            </p>
          </div>
          <ul className="space-y-3 text-sm text-slate-600 dark:text-slate-300">
            <li className="flex items-start gap-2">
              <CheckCircle2 className="mt-0.5 h-4 w-4 text-cyan-600 dark:text-cyan-300" />
              Suporte a MFA via TOTP, WebAuthn e tokens físicos.
            </li>
            <li className="flex items-start gap-2">
              <CheckCircle2 className="mt-0.5 h-4 w-4 text-cyan-600 dark:text-cyan-300" />
              Logs enriquecidos com origem, dispositivo e agente responsável.
            </li>
            <li className="flex items-start gap-2">
              <CheckCircle2 className="mt-0.5 h-4 w-4 text-cyan-600 dark:text-cyan-300" />
              Provisionamento automatizado via CLI ou APIs quando necessário.
            </li>
          </ul>
      ...
      <Card className="md:w-1/2">
        <CardHeader>
          <Badge tone="neutral" className="self-start">Escolha o fluxo</Badge>
          ...
          {modes.map(({ id, label }) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => setMode(id)}
                className={`flex flex-col items-center gap-1 rounded-2xl border px-3 py-3 text-xs font-semibold transition ${
                  isActive
