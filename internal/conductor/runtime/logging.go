...
func EventLog(ev Event) {
	...
	// if ok := logzN.NewDBusNotifier().EnabledFlag; ok {
	// 	notifier := logzN.NewDBusNotifier()
	// 	hostname, _ := os.Hostname()
	// 	pid := os.Getpid()
	// 	entry := NewEntry(logz.GetLoggerZ("Telemetry")).
	// 		WithProcessID(pid).
...
func RecordLatency(seconds float64) {
	...
	logz.Log("debug", fmt.Sprintf("latency observed: %.4fs", seconds))
}

// LogIntentAccepted logs when an intent is accepted for processing.
func LogIntentAccepted(intentID, path, method string) {
	logz.Log("info", fmt.Sprintf("intent accepted: id=%s path=%s method=%s time=%s",
