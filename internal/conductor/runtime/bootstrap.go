...
import (
	...
	// logzN "github.com/kubex-ecosystem/logz/api/notifiers"
)

// var wsNotifierConfig *logzN.NotifierWebSocketConfig
// var notifier *logzN.WebSocketNotifier

func init() {
	// inicia métricas/WS do logz (Prometheus + hub WS, se habilitado)
	// wsNotifierConfig = logzN.NewNotifierWebSocketConfig(
	// 	nil, //*tls.Config{},
	// 	2*time.Second,
	// 	nil,
...
func Init(router *gin.Engine, s scheduler.IScheduler) {
	...
	logz.Log("info", "[Conductor] initializing runtime...")

	// if notifier.EnabledFlag {
	// 	notifier.Enable()
	// 	notifier.AuthToken = kbx.GetEnvOrDefault("LOGZ_AUTH_TOKEN", "")
	// 	logz.Log("info", "[Conductor] WebSocket notifier enabled.")
	// }
	...
	logz.Log("info", "[Conductor] middleware attached, DCL runtime (logz-backed) ativo.")
}
