...
type ChannelBase[T any] struct {
	*l.LoggerZ              // Logger for this ChannelBase instance
...
func NewChannelBase[T any](name string, buffers int, logger *l.LoggerZ) ci.IChannelBase[any] {
	if logger == nil {
...
func (cb *ChannelBase[T]) Close() error {
	...
	defer cb.MuUnlock()
	if cb.Channel != nil {
		cb.LoggerZ.Log("info", "Closing channel for:", cb.Name)
...
func (cb *ChannelBase[T]) Clear() error {
	...
	defer cb.MuUnlock()
	if cb.Channel != nil {
		cb.LoggerZ.Log("info", "Clearing channel for:", cb.Name)
