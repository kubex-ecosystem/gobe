...
func SwaggerMain(dbService *services.DBServiceImpl, _ error) {
	// Initialize logger
	logz.Log("info", "Initializing Swagger...")
	rtr, err := router.NewRouter(
		...
		dbService,
		&kbx.InitArgs{},
		logz.GetLoggerZ("GoBE Swagger"),
		true,
	)
	if err != nil {
		logz.Log("fatal", "❌ Failed to initialize router:", err)
		return
	}
	...
	if err != nil {
		logz.Log("fatal", "❌ Failed to initialize CORS:", err)
		return
	}

	// Start server
	logz.Log("info", "🚀 GoBE API Server starting...")
	logz.Log("info", "📚 Swagger docs available at: http://localhost:8080/swagger/index.html")
	logz.Log("info", "🔍 API endpoints at: http://localhost:8080/api/v1")

	docs.SwaggerInfo.Host = "localhost:8080"
	docs.SwaggerInfo.Title = "GoBE API"

	if err := rtr.Start(); err != nil {
		logz.Log("fatal", "❌ Failed to start server:", err)
	}
}
